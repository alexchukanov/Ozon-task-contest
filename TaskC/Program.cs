//Task C, Парное программирование (10 баллов)

/* Task description
 
В компании работает n разработчиков, где n — четное число. Сумасшедший менеджер решил разбить всех разработчиков 
на команды по два человека.

Для этого он составил список всех разработчиков и назначил каждому из них номер по списку (от 1 до n) и значение 
ai  — уровень мастерства i -го в списке разработчика.
...
первый среди них имеет номер 1, его уровень мастерства a1=2, подходящими (с минимальной абсолютной разностью)
являются разработчики с номерами 2,3,4,5, первый среди них 2, таким образом первая команда — это разработчики 
с номерами 1 и 2;

оставшиеся разработчики теперь имеют номера [3,4,5,6], первый среди них 3, его уровень a3=3, 
разработчик с минимальной абсолютной разностью только один (номер 6), таким образом команда — разработчики 
с номерами 3  и 6;
...
n целых чисел a1,a2,…,an (1≤ai≤100), где ai— уровень мастерства i-го разработчика.

Выходные данные:
Для каждого набора входных данных выведите n2 строк, i-я строка должна содержать пару чисел — номер первого 
и второго разработчика в i -й команде в порядке, описанном в условии.
...
Dictionary<byte, byte> map = null;

List<string> list = new List<string>();

for (int i = 0; i < setSize; i++)
{
    var setLength = byte.Parse(Console.ReadLine());

    var setArr = Console.ReadLine().Split(' ').Select(it => byte.Parse(it)).ToList();

    map = new Dictionary<byte, byte>();

    for (byte j = 0; j < setArr.Count; j++)
    {
        map[(byte)(j + 1)] = setArr[j];
    }

    FindMasterNumArr(map);
...
void FindMasterNumArr(Dictionary<byte, byte> map)
{
    byte first = map.ElementAt(0).Value;
    int min = int.MaxValue;
    byte ind = 0;

    for (int i = 1; i <= map.Count - 1; i++)
    {
        byte val = map.ElementAt(i).Value;

        int delta = Math.Abs(first - val);
        if (delta < min)
        {
            min = delta;
            ind = map.ElementAt(i).Key;
        }
    }

    list.Add($"{map.ElementAt(0).Key} {ind}");
